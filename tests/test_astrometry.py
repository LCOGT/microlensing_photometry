...
def test_find_images_shifts():
    ...
    reference = image_tools.build_image.fn(star_positions, [1]*len(randomX), (size,size), image_fraction = 0.25,
                                        star_limit = 1000)
    ...
    image = image_tools.build_image.fn(star_positions2, [1]*len(randomX), (size,size), image_fraction = 0.25,
                                        star_limit = 1000)

    shiftX, shiftY = lcowcs.find_images_shifts.fn(reference,image,image_fraction =0.25, upsample_factor=1)
